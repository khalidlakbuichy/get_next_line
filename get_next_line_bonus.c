...
char	*make_nl(t_list **head, int *found)
{
	char	*line;
	char	*carry;
	t_list	*iter;
...
char	*get_next_line(int fd)
{
	...
	{
		if (data[fd].found)
			return (make_nl(&(data[fd].head), &data[fd].found));
		else
		{
			buff = (char *)malloc(sizeof(char) * (BUFFER_SIZE + 1));
			if (!buff && ft_lst_clear(&(data[fd].head)))
				return (NULL);
			nb_read = read(fd, buff, BUFFER_SIZE);
			if (nb_read == -1)
				return (free(buff), ft_lst_clear(&(data[fd].head)), NULL);
			if (nb_read == 0)
				return (free(buff), make_nl(&data[fd].head, &data[fd].found));
			buff[nb_read] = '\0';
			ft_lst_append(&(data[fd].head), buff, nb_read);
			data[fd].found = is_found(buff);
		}
	}
}
